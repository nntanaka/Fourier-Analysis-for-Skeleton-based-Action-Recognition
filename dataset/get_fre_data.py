...
def get_all_fre_noise(U, num_sub):
    ...
    temporal_spectral = np.ones((2, 3, 25, 64), dtype=np.complex128)
    temporal_spectral[:, :, :, 1:32] = (1+1j)/np.sqrt(2)
    temporal_spectral[:, :, :, 33:] = (1-1j)/np.sqrt(2)
...
if __name__ == '__main__':
    ...
    for index in tqdm(range(N)):
        ...
        all_noise = get_all_fre_noise(Q, num_sub)
    ...
    np.savez('ntu60/frequency_noise/{:s}/{:s}/{:s}_band_{:d}'.format(arg.direction, arg.input_type, arg.band, arg.band_width), x_test=all_data, y_test=label)
