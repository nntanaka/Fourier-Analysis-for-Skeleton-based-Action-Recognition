...
def spatial_and_temporal_filter_noise(data_numpy, U, band_width, spatial_band, temporal_band):
    # data C T V M
...
if __name__ == '__main__':
    ...
    for index in tqdm(range(N)):
        ...
        data_numpy += spatial_and_temporal_filter_noise(all_noise, Q, arg.band_width, arg.band, arg.band)
            
        data_list.append(data_numpy)
        
    all_data = np.stack(data_list)
    
    np.savez('./data/ntu/frequency_noise/{:s}/{:s}_band_{:d}'.format(arg.input_type, arg.band, arg.band_width), x_test=all_data, y_test=label)