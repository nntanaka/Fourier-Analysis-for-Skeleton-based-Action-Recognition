...
if __name__ == '__main__':
    ...
    del npz_data
    npz_data = np.load('./adversarial_examples_bl/stgcn_free_bl_bonevel_falseAE_01.npz')#npz_data = np.load('ntu60/cs_occluded/occluded_part/occluded_part_ind_5.npz')
    data2, indices, true_indices = npz_data['x_test'], npz_data['indices'], npz_data['true_indices']
    del npz_data
    data1 = data1[true_indices, ...]
    data1 = data1[indices, ...]
    N, T, _ = data1.shape
    ...
    for index in tqdm(range(N)):
        ...
        if (data_numpy[1] == np.zeros((C, T, V), np.float32)).all():
            ...
            spatial_temporal_spectral = get_temporal_spectral(skes_data=np.stack([spatial_spectral['ax_1'], spatial_spectral['ax_2'], spatial_spectral['ax_3']]), sub_num=1)
    ...
    with open('./adversarial_examples/bv_stgcn_free_attack_spatial.pkl', 'wb') as f:
        pickle.dump(spatial_dict, f)
        
    with open('./adversarial_examples/bv_stgcn_free_attack_temporal.pkl', 'wb') as f:
        pickle.dump(temporal_dict, f)
    
    with open('./adversarial_examples/bv_stgcn_free_attack_spatial_temporal.pkl', 'wb') as f:
