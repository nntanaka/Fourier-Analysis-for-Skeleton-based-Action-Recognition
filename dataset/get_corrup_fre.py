...
parser.add_argument('--corrupted_data', default='./data/ntu/cs_occluded/gaussian_noise/gaussian_noise_std_005.npz',
                    help='location of currpted dataset npz file')
...
if __name__ == '__main__':
    ...
    del npz_data
    npz_data = np.load(arg.corrupted_data)
    data2 = npz_data['x_test']
    del npz_data
    N, T, _ = data1.shape
    print(data2.shape)
    data1 = data1.reshape((N, T, 2, 25, 3)).transpose(0, 4, 1, 3, 2) # N C T V M
    data2 = data2.reshape((N, T, 2, 25, 3)).transpose(0, 4, 1, 3, 2) # N C T V M
    ...
    for index in tqdm(range(N)):
        ...
        data_numpy2 = valid_crop_resize(data_numpy2, valid_frame_num2, p_interval, window_size)
        if arg.bone:
    ...
    with open('./joint_corruption_spectrum.pkl', 'wb') as f:
