...
def fourier_noise(norm, s_ind, t_ind, U):
    # data M C V T
    temporal_spectral = np.zeros((2, 3, 25, 64), np.float32)
    temporal_spectral[:, :, s_ind, t_ind] = 1
    temporal_spectral[:, :, s_ind, -t_ind] = 1
    ...
    data_numpy = (norm/np.linalg.norm(data_numpy.reshape(-1), ord=2)) * data_numpy
    
    return data_numpy

def fourier_noise2(norm, s_ind, t_ind, U, s_range, t_range):
    # data M C V T
    temporal_spectral = np.zeros((2, 3, 25, 64), np.float32)
    temporal_spectral[:, :, s_ind*s_range:(s_ind+1)*s_range, t_ind*t_range:(t_ind+1)*t_range] = 1
    if t_ind==0:
        temporal_spectral[:, :, s_ind*s_range:(s_ind+1)*s_range, -(t_ind+1)*t_range+1:] = 1
    else:
        temporal_spectral[:, :, s_ind*s_range:(s_ind+1)*s_range, -(t_ind+1)*t_range+1:-t_ind*t_range+1] = 1
